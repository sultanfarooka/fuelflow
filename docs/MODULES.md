...
**Last Updated:** 2026-06-18 (M08-F08 Fuel Type Management — In Progress: management surface on the Station Configuration Fuel Types tab)
...
### M08-F08 — Fuel Type Management   [Status: In Progress]
...
| ID | Requirement | Legacy | Status |
|---|---|---|---|
| M08-F08-R01 | List all fuel types for the station: name, unit, source (OMC catalog vs custom), active/inactive status, and references (tank count + whether an active price exists; nozzles hang off tanks). Visible to Owner + Manager only | — | In Progress |
| M08-F08-R02 | Add a fuel type post-onboarding — select from the OMC catalog or create a custom type (name + unit); duplicate names rejected per station. Supersedes M02-F01-R03 | — | In Progress |
| M08-F08-R03 | Rename a fuel type's display name (OMC-derived and custom alike — both are per-station rows, so renaming affects only this station; the shared OMC catalog is untouched). Display-name only — historical records unaffected | — | In Progress |
| M08-F08-R04 | Activate / deactivate a fuel type. Deactivated types are excluded from new price, tank, and nozzle pickers but retained for historical reporting | — | In Progress |
| M08-F08-R05 | Deactivation blocked (`409` + blocking references) while the type is referenced by a tank or an active price. Fuel types are never hard-deleted on the management surface (the onboarding wizard's remove-just-added DELETE is unchanged) | — | In Progress |
| M08-F08-R06 | Adding a fuel type does not auto-create a price/tank/nozzle; the UI flags it "not yet sellable" until it has an active price and ≥1 tank (mirrors [M12-F01-R15](#m12-f01--onboarding-wizard) completeness checks) | — | In Progress |
| M08-F08-R07 | Add / rename / activate / deactivate are audit-logged (actor, station, before/after) — via Serilog now; persisted to the `AuditLog` table when [M01-F08](#m01-f08--audit-trail) ships | — | In Progress |
