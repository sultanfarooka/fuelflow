...
Per-feature Software Requirements Document. Top index: [`../SRD.md`](../SRD.md).
...
docs/srd/
├── README.md                 # this file
└── MXX-<kebab>/              # one directory per module
...
| Drop entirely | File: `lifecycle: removed`, `removed_reason:`, `removed_at:`. Module index renders struck through; link still works. |

## Relationship to other docs

| Doc | Answers | Lifecycle |
|---|---|---|
| SRD feature file (here) | *What* + *why* + when "done" (AC). | Stable; rarely changes after `spec-locked`. |
...
**MODULES.md transition:** new features → SRD only. Migrated features → MODULES.md row becomes `→ moved to <SRD path>` in the same PR that adds the SRD. Untouched modules stay in MODULES.md until their own migration pass. Root [`CLAUDE.md`](../../CLAUDE.md) Rule 1 reads "locate in SRD or MODULES.md" during this window.
