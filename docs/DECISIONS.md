...
| D-04 | Self-host `capcom6/sms-gateway` (Docker + paired Android relay device + Firebase Cloud Messaging) for [M01-F09](MODULES.md#m01-f09--phone-first-authentication) SMS OTP delivery, instead of Twilio / Jazz / Telenor. | Cost ceiling matters for a free-trial-driven Pakistani SaaS; per-SMS pricing on commercial providers makes signup fan-out expensive. Self-hosted gateway runs on existing infra and reuses a real SIM card for genuine PK-network delivery. Setup is documented in [`server/sms-gateway/README.md`](../server/sms-gateway/README.md). | Multi-region deployment makes single-device delivery a bottleneck, regulatory pressure requires a licensed sender, or SMS volume grows past what one paired relay can sustain — at which point switch to a commercial PK provider via `Sms:Gateway:BaseUrl` + alt impl. |
