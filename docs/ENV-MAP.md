...
|---|---|---|---|---|
| `ConnectionStrings:DefaultConnection` | [`server/FuelFlow.Infrastructure/DependencyInjection.cs`](../server/FuelFlow.Infrastructure/DependencyInjection.cs) | **Yes** | Server only | PostgreSQL connection string. Dev format: `Host=localhost;Port=5432;Database=fuelflow_dev;Username=fuelflow;Password=…` (host 5432 — the container's published port; native Postgres often owns 5432). Supplied locally via user-secret or a gitignored `appsettings.Development.json`; must use 5432. |
