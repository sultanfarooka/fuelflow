...
function TankRow({
  ...
  const entryCount =
    dipRes?.data?.entries?.length ?? tank.dipChartEntryCount ?? 0;
...
export function StepTanks({ stationId, onNext, onBack }: Props) {
  ...
  const effectiveShowAddForm =
    showAddForm || (tanks.length === 0 && fuelTypes.length > 0);
  ...
  const fuelTypeNameById = (id: string) =>
    ...
    tanks.find((t) => t.fuelTypeId === id)?.fuelTypeName ??
    "";
  ...
  const allFuelTypesCovered =
    fuelTypes.length > 0 &&
    fuelTypes.every((ft) => tanks.some((t) => t.fuelTypeId === ft.id));
        ...
        <CardContent className="space-y-4">
          {fuelTypes.length === 0 ? (
            <p className="text-sm text-muted-foreground">
              {t("onboarding.step4.noFuelTypes")}
            </p>
          ) : (
            <>
              {tanks.length > 0 && (
                <div
                  className="space-y-2"
                  role="list"
                  aria-label={t("onboarding.step4.tankList")}
                >
                  {tanks.map((tank) => (
                    <TankRow
                      key={tank.id}
              ...
              {tanks.length === 0 && !showAddForm && (
                <p className="text-sm text-muted-foreground">
                  {t("onboarding.step4.emptyTanks")}
                </p>
              )}

              <div
                className={cn(
                  tanks.length > 0 && "border-t border-border pt-4",
                )}
              >
                {!effectiveShowAddForm ? (
                  ...
                  <div className="space-y-4 rounded-lg border border-dashed border-border p-4 text-sm">
                    <p className="font-medium">
                      {t("onboarding.step4.addTankFormTitle")}
                    </p>
                    <FieldGroup>
                      <Field>
                        ...
                        >
                          <SelectTrigger id="tank-fuel-type">
                            <SelectValue
                              placeholder={t("onboarding.step4.selectFuelType")}
                            />
                          </SelectTrigger>
                          <SelectContent>
                            {fuelTypes.map((ft) => (
